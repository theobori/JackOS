...
void bios32_init()
{
    gdt_set_entry(6, 0, 0xffffffff, 0x9A, 0x0f);        // Adding two more entries to the GDT
    gdt_set_entry(7, 0, 0xffffffff, 0x92, 0x0f);
    gdt32.base = (u32)g_gdt;                            // Creating new gdt
...
void bios32_service(u8 interrupt, registers_16_t *in, registers_16_t *out)
{
    ...
    gdt32.base = (u32)REBASE_ADDRESS((&gdt_entries_32));                    // set base of gdt_entries_32(bios32_call.asm) starting from 0x7c00
    memcpy(&bios32_gdt_ptr, &gdt32, sizeof(idt_register_t));                // copy gdt32 to bios32_gdt_ptr(bios32_call.asm)
